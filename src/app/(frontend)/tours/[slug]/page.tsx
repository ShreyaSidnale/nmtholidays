      ...
      <div className="container-page py-8">
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_360px]">
          ...
          <div className="min-w-0 space-y-12">
            {tour.overview && (
              <section>
                <h2 className="mb-3 text-2xl font-bold text-ink">{tt('overview')}</h2>
            ...
            {tour.highlights?.length ? (
              <section>
                <h2 className="mb-3 text-2xl font-bold text-ink">{tt('highlights')}</h2>
            ...
            {tour.itinerary?.length ? (
              <section>
                <h2 className="mb-5 text-2xl font-bold text-ink">{tt('itinerary')}</h2>
              ...
              <section className="grid gap-6 sm:grid-cols-2">
                {tour.inclusions?.length ? (
                  <div className="rounded-[var(--radius-card)] border border-slate-100 bg-white p-5 shadow-sm">
                    <h3 className="mb-3 font-bold text-ink">{tt('included')}</h3>
                ...
                {tour.exclusions?.length ? (
                  <div className="rounded-[var(--radius-card)] border border-slate-100 bg-white p-5 shadow-sm">
                    <h3 className="mb-3 font-bold text-ink">{tt('notIncluded')}</h3>
                    <ul className="space-y-2 text-sm text-slate-600">
            ...
            {tour.faqs?.length ? (
              <section>
                <h2 className="mb-5 text-2xl font-bold text-ink">{tt('faqs')}</h2>
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-[var(--radius-card)] border border-slate-200 bg-white p-6 shadow-lg">
              {tour.priceFrom != null && (
                <div className="mb-4">
                  <span className="text-sm text-slate-500">{tt('startingFrom')}</span>
                  <div className="text-3xl font-extrabold text-brand-700">
                    {formatPrice(tour.priceFrom, tour.currency ?? 'INR')}
                    <span className="text-sm font-medium text-slate-500"> {tt('perPerson')}</span>
                  </div>
                </div>
              )}
              <WhatsAppButton
              ...
              <div className="my-5 flex items-center gap-3 text-xs text-slate-400">
                <span className="h-px flex-1 bg-slate-200" /> {tt('orEnquire')} <span className="h-px flex-1 bg-slate-200" />
              </div>
